...
const AdminControl = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5002/filter/${filter}`,
        token && {
          headers: {
            "auth-token": token,
          },
  ...
  const deleteValue = async (id) => {
    try {
      axios.patch(`http://localhost:5002/deactive/${id}`).then((result) => {
  ...
  const activeValue = async (id) => {
    try {
      axios.patch(`http://localhost:5002/active/${id}`).then((result) => {
        if (result.status === 200) {
          setStatus(true);
        }
      });
  ...
  const upDate = async (id) => {
    ...
    try {
      ...
      const res = await axios.patch(
        `${port}/${id}`,
        { email: email, fullname: fullname, category: categoryid },
        token && {
          headers: {
            "auth-token": token,
          },
  ...
  return (
    <>
      ...
      <>
        ...
        <center>
          <button onClick={newData} className="m-2 p-2 btn btn-primary">
            {addData ? "Do not add" : "Add New Data"}
          </button>
        </center>
        <center>
        ...
        <center>
          {" "}
          <button onClick={showAllData} className="btn btn-primary m-2 p-2">
            All Data
          </button>
        </center>
        <select
        ...
        >
          {fitlerDrop &&
            fitlerDrop.map((val, index) => {
              return (
                <option value={val._id} className="bg-white">
                  {val.category}
                </option>
              );
            })}
                  ...
                  <div key={index}>
                    {edit ? (
                      <>
                        <center>
                          <span>Name:</span>
                          <p className={val.active ? "text-success" : "line"}>
                            {val.fullname}
                          </p>
                        </center>
                        <center>
                      ...
                      <>
                        {idCheck === val._id ? (
                          <>
                            ...
                            <br />
                            <select
                              className="dropdown mt-2 container-fluid p-2 bg-secondary d-flex m-auto w-50"
