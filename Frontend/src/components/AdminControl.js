...
const AdminControl = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:5000/filter/${filter}`, token && {
            headers: {
                "auth-token":
                    token
            },
    ...
    const deleteValue = async (id) => {
        ...
        try {
            axios.patch(`http://localhost:5000/deactive/${id}`)
                .then(result => {
    ...
    const activeValue = async (id) => {
        ...
        try {
            axios.patch(`http://localhost:5000/active/${id}`)
                .then(result => {
                    if (result.status === 200) {
                        setStatus(true)
                    }
    ...
    useEffect(()=>{
        const token=sessionStorage.getItem("auth-token")
        axios.post(`http://localhost:5000/${search}`, token && {
            headers: {
                "auth-token":
                    token
            },
    ...
    return (
        <>
            ...
            <>
                ...
                <center><button onClick={newData} className="mt-2 p-2 btn btn-primary">{addData ? 'Hide' : 'Add New Data'}</button></center><br/>
                   <center>
                ...
                <center> <button onClick={showAllData} className="btn btn-success m-2 p-2">All Data</button></center>
                <select
                    className="dropdown mt-2  p-2 bg-secondary d-flex m-auto w-50"
                    ...
                    return (
                        <>
                            {val.role === 0 ? <div key={index}>
                                {edit ? <>
                                    <center ><span><u>Name:</u></span><p className={val.active ? 'text-success' : 'line'}>{val.fullname}</p></center>
                                    <center><span><u>Email :</u></span><p className={val.active ? 'text-success' : 'line'}>{val.email}</p></center>
                                    <center><span><u>Category :</u></span><p className={val.active ? 'text-success' : 'line'}>{val.category}</p></center>
                                </> : <>{idCheck === val._id ?
                                    <>
                                ...
                                <center>
                                    {val.active ? <button onClick={() => deleteValue(val._id)} className="btn btn-danger mt-2 mr-2">Deactivate User</button> 
                                    : <button onClick={() => activeValue(val._id)} className="btn btn-success mt-2 mr-2">Activate User</button>}


                                    {edit ? <button onClick={() => Edit(val._id, val.email)} className="btn btn-warning mt-2" disabled={val.active ?
